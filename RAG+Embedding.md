...
python的数据分析三剑客有哪些，怎么使用，使用场景是什么



embedding只负责将数据转向量的计算，不负责存储，计算出来的向量数据需要自己进行存储，如果需要进行分类聚合检索等操作，需要将写公式计算余玄距离。
...
向量数据库不等于关系性数据库，类似关系型数据库

向量数据库主要就负责两件事情：1、向量存储，2、向量检索



数据量比较小的情况下使用：FAISS(未开源，少量数据的情况下性能不错，可以支持CPU/GPU)、Chroma

如果考虑数据量大和私密性：Milvus(开源，分布式多节点部署，适合大项目)



faiss数据库的documents组成 {page_content:'', mate_data:{'xx':xx}}，mate_data里的数据不能支持语义检索，只能支持过滤检索。
